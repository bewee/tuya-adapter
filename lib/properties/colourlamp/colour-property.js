...
function hsl2lamp(hsl, format) {
  switch (format) {
    ...
    case 'HEXHLS':
      return `${rgb2hex(hsl2rgb([hsl[0], hsl[1], hsl[2]/2])).substr(1)}${pad(4, Math.round(hsl[0]).toString(16))}${pad(2, Math.round(hsl[2]*255/100).toString(16))}${pad(2, Math.round(hsl[1]*255/100).toString(16))}`;
  }
  throw 'Unknown colour format!';
}
...
class ColourProperty extends TuyaProperty {
  constructor(device, cnf) {
    ...
    if (this.device.ownconf.format != 'HSL' && this.device.ownconf.format != 'HEXHLS')
      console.warn(this.id, 'Unknown colour format');
  }
