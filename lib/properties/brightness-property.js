...
class BrightnessProperty extends TuyaProperty {
  constructor(device, cnf) {
    ...
    if (!this.device.ownconf.minbrightness || !this.device.ownconf.maxbrightness) {
      if (!this.device.ownconf.minbrightness) {
        console.info(this.id, 'Set minbrightness to default 0');
        this.device.ownconf.minbrightness = 0;
      }
      if (!this.device.ownconf.maxbrightness) {
        console.info(this.id, 'Set maxbrightness to default 1000');
        this.device.ownconf.maxbrightness = 1000;
      }
