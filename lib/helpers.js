...
function remap(x, min_from, max_from, min_to, max_to, multipleOf) {
  const raw = (x-min_from) / (max_from - min_from) * (max_to-min_to) + min_to;
  if (multipleOf)
    return Math.round(raw/multipleOf) * multipleOf;
  else
    return raw;
}

module.exports = {hsl2rgb: hsl2rgb, rgb2hsl: rgb2hsl, pad: pad, rgb2hex: rgb2hex, hex2rgb: hex2rgb, remap: remap};
