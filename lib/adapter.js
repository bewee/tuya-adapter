      ...
      for (const conf of this.config.devices) {
        ...
        switch (conf.type) {
          case 'Plug':
            ...
            break;
          case '1 gang switch':
            d = new SwitchDevice(this, conf.id, conf.key, conf.name&&conf.name!='' ? conf.name : null, 1);
            break;
          case '2 gang switch':
            d = new SwitchDevice(this, conf.id, conf.key, conf.name&&conf.name!='' ? conf.name : null, 2);
            break;
          case '3 gang switch':
            d = new SwitchDevice(this, conf.id, conf.key, conf.name&&conf.name!='' ? conf.name : null, 3);
            break;
          case 'Lamp':
